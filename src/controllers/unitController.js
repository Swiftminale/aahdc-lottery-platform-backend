...
export async function submitUnits(req, res) {
  try {
    ...
    if (!Array.isArray(units) || units.length === 0) {
      return res.status(400).json({ message: 'Invalid or empty unit submission.' });
    }
    ...
    for (const unit of units) {
      if (!unit.unitId || !unit.typology || unit.netArea == null || unit.grossArea == null || unit.floorNumber == null || !unit.blockName || unit.totalBuildingGrossArea == null) {
        return res.status(400).json({ message: `Missing required fields for unit: ${JSON.stringify(unit)}` });
      }
      if (unitIds.has(unit.unitId)) {
        return res.status(400).json({ message: `Duplicate Unit ID found: ${unit.unitId}` });
      }
      unitIds.add(unit.unitId);

      if (!buildingGrossAreas[unit.blockName]) {
        buildingGrossAreas[unit.blockName] = { declaredTotal: unit.totalBuildingGrossArea, calculatedTotal: 0 };
      }
      buildingGrossAreas[unit.blockName].calculatedTotal += parseFloat(unit.grossArea); // Ensure number conversion
    }
    ...
    for (const blockName in buildingGrossAreas) {
      ...
      if (Math.abs(declaredTotal - calculatedTotal) > 0.01) {
        return res.status(400).json({ message: `Gross area mismatch for block ${blockName}: Declared total ${declaredTotal}, Calculated sum of units ${calculatedTotal}.` });
      }
    }
    ...
    if (existingUnits.length > 0) {
      ...
      return res.status(409).json({ message: `One or more units already exist in the database: ${existingIds.join(', ')}` });
    }

    const createdUnits = await Unit.bulkCreate(units);
    res.status(201).json({ message: 'Units submitted successfully', units: createdUnits });
