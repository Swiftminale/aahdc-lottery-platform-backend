...
export async function checkPreAllocationCompliance(units) {
  ...
  const totalResidentialArea = residentialUnits.reduce((sum, u) => sum + u.grossArea, 0);

  const typologyArea = {
    Studio: residentialUnits.filter(u => u.typology === 'Studio').reduce((sum, u) => sum + u.grossArea, 0),
    '1BR': residentialUnits.filter(u => u.typology === '1BR').reduce((sum, u) => sum + u.grossArea, 0),
    '2BR': residentialUnits.filter(u => u.typology === '2BR').reduce((sum, u) => sum + u.grossArea, 0),
    '3BR': residentialUnits.filter(u => u.typology === '3BR').reduce((sum, u) => sum + u.grossArea, 0),
  };

  if (totalResidentialArea > 0) {
    if ((typologyArea.Studio / totalResidentialArea) * 100 > 15.1) { // Allow slight tolerance
      complianceIssues.push("Overall Studio typology exceeds 15% of residential gross area.");
    }
    if ((typologyArea['1BR'] / totalResidentialArea) * 100 > 40.1) {
      complianceIssues.push("Overall 1BR typology exceeds 40% of residential gross area.");
    }
    if ((typologyArea['2BR'] / totalResidentialArea) * 100 > 25.1) {
      complianceIssues.push("Overall 2BR typology exceeds 25% of residential gross area.");
    }
    if ((typologyArea['3BR'] / totalResidentialArea) * 100 > 20.1) {
      complianceIssues.push("Overall 3BR typology exceeds 20% of residential gross area.");
    }
  }
  ...
  const totalCommercialArea = commercialUnits.reduce((sum, u) => sum + u.grossArea, 0);
  if (commercialUnits.length > 0 && totalCommercialArea * 0.30 === 0) {
      complianceIssues.push("Commercial units submitted but impossible to meet 30% AAHDC share for shops (area too small).");
  }
  ...
  if (!isConstructionComplete) {
    complianceIssues.push("Clause 7.9 violation: 30% physical construction is not yet complete.");
  }
  if (!isDistributionAgreementSigned) {
    complianceIssues.push("Schedule 7 violation: Distribution agreement has not been signed.");
  }
...
export async function checkPostAllocationCompliance(aahdcUnits, developerUnits) {
    ...
    const totalGrossArea = allUnits.reduce((sum, unit) => sum + unit.grossArea, 0);
    const aahdcGrossArea = aahdcUnits.reduce((sum, unit) => sum + unit.grossArea, 0);

    // Clause 5.6: 30% gross area must be transferred
    if (totalGrossArea > 0 && (aahdcGrossArea / totalGrossArea) * 100 < 29.9) { // Using 29.9 to allow for floating point inaccuracies
      complianceIssues.push(`Clause 5.6 violation: AAHDC's allocated gross area is less than 30%. Current: ${(aahdcGrossArea / totalGrossArea * 100).toFixed(2)}%`);
    }

    // Typological caps respected for AAHDC's allocation
    const aahdcResidentialUnits = aahdcUnits.filter(u => u.typology !== 'Shop');
    const aahdcTotalResidentialArea = aahdcResidentialUnits.reduce((sum, u) => sum + u.grossArea, 0);

    const aahdcTypologyArea = {
      Studio: aahdcResidentialUnits.filter(u => u.typology === 'Studio').reduce((sum, u) => sum + u.grossArea, 0),
      '1BR': aahdcResidentialUnits.filter(u => u.typology === '1BR').reduce((sum, u) => sum + u.grossArea, 0),
      '2BR': aahdcResidentialUnits.filter(u => u.typology === '2BR').reduce((sum, u) => sum + u.grossArea, 0),
      '3BR': aahdcResidentialUnits.filter(u => u.typology === '3BR').reduce((sum, u) => sum + u.grossArea, 0),
    };

    if (aahdcTotalResidentialArea > 0) {
      if ((aahdcTypologyArea.Studio / aahdcTotalResidentialArea) * 100 > 15.1) {
        complianceIssues.push(`AAHDC Studio typology allocation exceeds 15% of residential gross area: ${(aahdcTypologyArea.Studio / aahdcTotalResidentialArea * 100).toFixed(2)}%`);
      }
      if ((aahdcTypologyArea['1BR'] / aahdcTotalResidentialArea) * 100 > 40.1) {
        complianceIssues.push(`AAHDC 1BR typology allocation exceeds 40% of residential gross area: ${(aahdcTypologyArea['1BR'] / aahdcTotalResidentialArea * 100).toFixed(2)}%`);
      }
      if ((aahdcTypologyArea['2BR'] / aahdcTotalResidentialArea) * 100 > 25.1) {
        complianceIssues.push(`AAHDC 2BR typology allocation exceeds 25% of residential gross area: ${(aahdcTypologyArea['2BR'] / aahdcTotalResidentialArea * 100).toFixed(2)}%`);
      }
      if ((aahdcTypologyArea['3BR'] / aahdcTotalResidentialArea) * 100 > 20.1) {
        complianceIssues.push(`AAHDC 3BR typology allocation exceeds 20% of residential gross area: ${(aahdcTypologyArea['3BR'] / aahdcTotalResidentialArea * 100).toFixed(2)}%`);
      }
    }
    ...
    const aahdcCommercialArea = aahdcCommercialUnits.reduce((sum, u) => sum + u.grossArea, 0);
    const totalProjectCommercialArea = totalCommercialUnits.reduce((sum, u) => sum + u.grossArea, 0);

    if (totalProjectCommercialArea > 0 && (aahdcCommercialArea / totalProjectCommercialArea) * 100 < 29.9) {
        complianceIssues.push(`AAHDC Shop allocation is less than 30% of total commercial area. Current: ${(aahdcCommercialArea / totalProjectCommercialArea * 100).toFixed(2)}%`);
    }
